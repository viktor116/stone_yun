...
public class WitherSkeletonMerchantScreen extends HandledScreen<WitherSkeletonInteractionHandler> {
    ...
    private final WitherSkeletonMerchantScreen.WidgetButtonPage[] offers = new WitherSkeletonMerchantScreen.WidgetButtonPage[7];
    ...
    private void syncRecipeIndex() {
        ((WitherSkeletonInteractionHandler)this.handler).setRecipeIndex(this.selectedIndex);
        ((WitherSkeletonInteractionHandler)this.handler).switchTo(this.selectedIndex);
    ...
    protected void init() {
        ...
        for(int l = 0; l < 7; ++l) {
            this.offers[l] = (WitherSkeletonMerchantScreen.WidgetButtonPage)this.addDrawableChild(new WitherSkeletonMerchantScreen.WidgetButtonPage(i + 5, k, l, (button) -> {
                if (button instanceof WitherSkeletonMerchantScreen.WidgetButtonPage) {
                    this.selectedIndex = ((WitherSkeletonMerchantScreen.WidgetButtonPage)button).getIndex() + this.indexStartOffset;
...
//        TradeOfferList tradeOfferList = ((WitherSkeletonInteractionHandler)this.handler).getRecipes();
    ...
    private void drawLevelInfo(DrawContext context, int x, int y, TradeOffer tradeOffer) {
        int i = ((WitherSkeletonInteractionHandler)this.handler).getLevelProgress();
        int j = ((WitherSkeletonInteractionHandler)this.handler).getExperience();
        if (i < 5) {
            ...
            if (j >= k && VillagerData.canLevelUp(i)) {
                ...
                int n = ((WitherSkeletonInteractionHandler)this.handler).getMerchantRewardedExperience();
                if (n > 0) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!tradeOfferList.isEmpty()) {
            ...
            while(true) {
                ...
                if (((WitherSkeletonInteractionHandler)this.handler).isLeveled()) {
                    this.drawLevelInfo(context, i, j, tradeOffer);
                }

                if (tradeOffer.isDisabled() && this.isPointWithinBounds(186, 35, 22, 21, (double)mouseX, (double)mouseY) && ((WitherSkeletonInteractionHandler)this.handler).canRefreshTrades()) {
                    context.drawTooltip(this.textRenderer, DEPRECATED_TEXT, mouseX, mouseY);
                }

                WitherSkeletonMerchantScreen.WidgetButtonPage[] var19 = this.offers;
                int var20 = var19.length;

                for(int var21 = 0; var21 < var20; ++var21) {
                    WitherSkeletonMerchantScreen.WidgetButtonPage widgetButtonPage = var19[var21];
                    if (widgetButtonPage.isSelected()) {
                        widgetButtonPage.renderTooltip(context, mouseX, mouseY);
                    }

                    widgetButtonPage.visible = widgetButtonPage.index < ((WitherSkeletonInteractionHandler)this.handler).getRecipes().size();
                }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        int i = ((WitherSkeletonInteractionHandler)this.handler).getRecipes().size();
        if (this.canScroll(i)) {
            int j = i - 7;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int i = (this.width - this.backgroundWidth) / 2;
        int j = (this.height - this.backgroundHeight) / 2;
        if (this.canScroll(((WitherSkeletonInteractionHandler)this.handler).getRecipes().size()) && mouseX > (double)(i + 94) && mouseX < (double)(i + 94 + 6) && mouseY > (double)(j + 18) && mouseY <= (double)(j + 18 + 139 + 1)) {
            this.scrolling = true;
        }
    ...
    class WidgetButtonPage extends ButtonWidget {
        ...
        public void renderTooltip(DrawContext context, int x, int y) {
            if (this.hovered && ((WitherSkeletonInteractionHandler)WitherSkeletonMerchantScreen.this.handler).getRecipes().size() > this.index + WitherSkeletonMerchantScreen.this.indexStartOffset) {
                ItemStack itemStack;
                if (x < this.getX() + 20) {
                    itemStack = ((TradeOffer)((WitherSkeletonInteractionHandler)WitherSkeletonMerchantScreen.this.handler).getRecipes().get(this.index + WitherSkeletonMerchantScreen.this.indexStartOffset)).getDisplayedFirstBuyItem();
                    context.drawItemTooltip(WitherSkeletonMerchantScreen.this.textRenderer, itemStack, x, y);
                } else if (x < this.getX() + 50 && x > this.getX() + 30) {
                    itemStack = ((TradeOffer)((WitherSkeletonInteractionHandler)WitherSkeletonMerchantScreen.this.handler).getRecipes().get(this.index + WitherSkeletonMerchantScreen.this.indexStartOffset)).getDisplayedSecondBuyItem();
                    if (!itemStack.isEmpty()) {
                        context.drawItemTooltip(WitherSkeletonMerchantScreen.this.textRenderer, itemStack, x, y);
                    }
                } else if (x > this.getX() + 65) {
                    itemStack = ((TradeOffer)((WitherSkeletonInteractionHandler)WitherSkeletonMerchantScreen.this.handler).getRecipes().get(this.index + WitherSkeletonMerchantScreen.this.indexStartOffset)).getSellItem();
                    context.drawItemTooltip(WitherSkeletonMerchantScreen.this.textRenderer, itemStack, x, y);
                }
            }
