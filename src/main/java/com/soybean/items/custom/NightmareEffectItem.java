...
/**
 * 梦魇药水物品，食用后会在玩家周围5格内产生从中心向外扩散的黑色粒子效果
 */
public class NightmareEffectItem extends Item {
    ...
    private void startNightmareParticleEffect(PlayerEntity player) {
        ...
        ServerEachTickTaskManager.scheduleTask(EFFECT_DURATION, 1, () -> {
            ...
            // 粒子数量也随着进度增加
            int baseParticleCount = (int) (200 * progress);
            ...
            for (int i = 0; i < baseParticleCount; i++) {
                ...
                if (distanceRatio < 0.3) {
                    ...
                    // 中间区域混合使用两种粒子
                    serverWorld.spawnParticles(
                            player.getRandom().nextFloat() < 0.7 ? ParticleTypes.LARGE_SMOKE : ParticleTypes.SMOKE,
                            particleX, particleY, particleZ,
                            1,
                            0.03, 0.03, 0.03,
                            0.001
                    ...
                    // 外部区域主要使用普通烟雾粒子
                    serverWorld.spawnParticles(
                            player.getRandom().nextFloat() < 0.3 ? ParticleTypes.LARGE_SMOKE : ParticleTypes.SMOKE,
                            particleX, particleY, particleZ,
                            1,
                            0.04, 0.04, 0.04,
                            0.001
            ...
            if (progress > 0.4) {
                // 额外的密集粒子
                int extraParticles = (int)(120 * (progress - 0.4) / 0.6); // 最多120个额外粒子
            ...
            if (progress > 0.6) {
                int borderParticles = (int)(100 * (progress - 0.6) / 0.4); // 边界粒子数量
                ...
                for (int i = 0; i < borderParticles; i++) {
                    ...
                    // 交替使用两种粒子
                    serverWorld.spawnParticles(
                            i % 2 == 0 ? ParticleTypes.SMOKE : ParticleTypes.ASH,
                            particleX, particleY, particleZ,
                            1,
                            0.02, 0.02, 0.02,
                            0.001
            ...
            if (progress > 0.85) {
                int splashParticles = (int)(60 * (progress - 0.85) / 0.15);
