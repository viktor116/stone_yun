...
/**
 * 梦幻药水物品，食用后会在玩家视野前方产生从中心向外扩散的白色粒子效果
 */
    ...
    private void startDreamParticleEffect(PlayerEntity player) {
        ...
        ServerEachTickTaskManager.scheduleTask(EFFECT_DURATION, 1, () -> {
            ...
            for (int i = 0; i < baseParticleCount; i++) {
                ...
                // 根据进度选择不同比例的粒子类型
                boolean useEndRod = progress < 0.3 ? true : // 初期全部使用END_ROD
                                    progress < 0.7 ? player.getRandom().nextFloat() < 0.8 : // 中期80%是END_ROD
                                    player.getRandom().nextFloat() < 0.6; // 后期60%是END_ROD
                
                // 生成粒子
                serverWorld.spawnParticles(
                        useEndRod ? ParticleTypes.END_ROD : ParticleTypes.SNOWFLAKE,
            ...
            if (progress > 0.4) {
                ...
                for (int i = 0; i < extraParticles; i++) {
                    ...
                    // 在内部区域主要使用END_ROD粒子
                    serverWorld.spawnParticles(
                            player.getRandom().nextFloat() < 0.85 ? ParticleTypes.END_ROD : ParticleTypes.SNOWFLAKE,
                            particleX, particleY, particleZ,
                            1,
                            0.03, 0.03, 0.03,
                            0.001
            ...
            if (progress > 0.6) {
                ...
                for (int i = 0; i < borderParticles; i++) {
                    ...
                    for (int layer = 0; layer < 8; layer++) { // 增加到8层
                        ...
                        // 交替使用两种粒子
                        serverWorld.spawnParticles(
                                layer % 2 == 0 ? ParticleTypes.END_ROD : ParticleTypes.SNOWFLAKE,
            ...
            if (progress > 0.85) {
                ...
                for (int i = 0; i < splashParticles; i++) {
                    ...
                    // 随机选择粒子类型
                    serverWorld.spawnParticles(
                            player.getRandom().nextFloat() < 0.7 ? ParticleTypes.END_ROD : ParticleTypes.SNOWFLAKE,
