...
import com.soybean.block.custom.entity.HalfWhiteBedEntity;
...
public class HalfWhiteBlockEntityRenderer implements BlockEntityRenderer<HalfWhiteBedEntity> {
            private final ModelPart bedHead;
            private final ModelPart bedFoot;

            public HalfWhiteBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    ...
    @Override
    public void render(HalfWhiteBedEntity bedBlockEntity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        ...
        if (world != null) {
            ...
            if (blockState.get(BedBlock.PART) == BedPart.HEAD) {
                ...
                this.renderPart(
                        ...
                        this.bedHead, // 始终使用床头模型
                        (Direction)blockState.get(BedBlock.FACING),
                        spriteIdentifier,
                        k,
                        j,
                        false // isFoot 保持为 false，因为只渲染床头部分
                );
            }
            // 如果当前方块是床尾 (BedPart.FOOT)，则不进行任何渲染

        } else {
            // 如果世界信息为空 (如在物品栏中渲染)，我们假设它是一个床头并渲染
            // 您也可以选择在这里不渲染任何东西
